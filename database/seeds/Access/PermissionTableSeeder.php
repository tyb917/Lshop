...
class PermissionTableSeeder extends Seeder
{
    public function run()
    {
        ...
        if (env('DB_CONNECTION') == 'mysql') {
            ...
            DB::collection(config('access.permissions_table'))->delete();
            DB::collection(config('access.permissions_table'))->delete();
            DB::collection(config('access.permissions_table'))->delete();
        } else {
        ...
        $viewAccessManagement->display_name = '访问视图管理';
        ...
        $deactivateUser->display_name = '禁用用户';
        ...
        $reactivateUser->display_name = '恢复禁用用户';
