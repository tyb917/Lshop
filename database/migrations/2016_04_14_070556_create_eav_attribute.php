...
class CreateEavAttribute extends Migration
{
    ...
    public function up()
    {
        Schema::create('eav_attribute', function (Blueprint $table) {
            ...
            $table->char('attribute_code')->nullable()->comment('属性代码');
            $table->char('attribute_model')->nullable()->comment('属性模型');
            $table->char('backend_model')->nullable()->comment('后台模型');
            $table->char('backend_type',8)->default('static')->comment('后台类型');
            $table->char('frontend_model')->nullable()->comment('前台模型');
            $table->char('frontend_input',50)->nullable()->comment('前台输出类型');
            $table->char('frontend_label')->nullable()->comment('前台输出名称');
            $table->char('frontend_class')->nullable()->comment('前台输入验证');
            $table->char('source_model')->nullable()->comment('原始模型');
            ...
            $table->char('note')->nullable()->comment('备注');
